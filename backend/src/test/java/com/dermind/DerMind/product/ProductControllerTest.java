...
class ProductControllerTest {
    ...
    void getAllProducts_returnsOkWithList() throws Exception {
        when(productService.getAllProducts()).thenReturn(List.of(
                new ProductResponseDTO(1L, "Moisturizer", "LANEIGE", "Aloe Vera", 8.5)
        ));

        mockMvc.perform(get("/api/products"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(1))
                .andExpect(jsonPath("$[0].name").value("Moisturizer"));
    }
    ...
    void getProductById_existingProduct_returnsOk() throws Exception {
        when(productService.getProductById(1L)).thenReturn(
                new ProductDetailDTO(1L, "Toner", "COSRX", "Niacinamide", 9.0, 4.5, 10, 3)
        );
    ...
    void searchProducts_returnsMatchingProducts() throws Exception {
        when(productService.searchProducts("toner")).thenReturn(List.of(
                new ProductResponseDTO(2L, "Toner Pad", "COSRX", "AHA BHA", 8.0)
        ));

        mockMvc.perform(get("/api/products/search").param("q", "toner"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name").value("Toner Pad"));
    }
}
